...
def expand_crop_rotation_template(crop_json, crop_id, start_year, end_year):
    template = crop_json["cropRotationTemplates"][crop_id]
    rotation = []

    for base_year in range(start_year, end_year + 1):
        for crop_entry in template:
            entry = copy.deepcopy(crop_entry)

            for ws in entry["worksteps"]:
                if "date" in ws:
                    y, m, d = ws["date"].split("-")
                    real_year = base_year + int(y)
                    ws["date"] = f"{real_year:04d}-{m}-{d}"
...
def run_producer(server={"server": None, "port": None}, shared_id=None):
    ...
    config = {
        ...
        "crop-spinup.json": "crop_projection_N200.json",
        "crop-projection.json": "crop_projection_N0_cover.json",
        "site.json": "site_projection.json",
        "setups-file": "sim_setups_spinup.csv",
        "run-setups": "[1]",
        "shared_id": shared_id
    }
    ...
    for _, setup_id in enumerate(run_setups):
        ...
        # read spinup crop.json
        with open(setup.get("crop-spinup.json", config["crop-spinup.json"])) as _:
            crop_spinup_json = json.load(_)

        # read projection crop.json
        with open(setup.get("crop-projection.json", config["crop-projection.json"])) as _:
            crop_projection_json = json.load(_)

        use_vernalisation_fix = setup.get("use_vernalisation_fix", False)
        ...
        # with open(f"out/env_template_setup_{setup_id}.json", "w") as f:
