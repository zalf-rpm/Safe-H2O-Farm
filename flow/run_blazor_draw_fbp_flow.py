...
abs_imports = [str(PATH_TO_CAPNP_SCHEMAS.resolve())]
...
config = {
    ...
    "path_to_flow": "/home/berg/GitHub/Safe-H2O-Farm/safe_h2o_farm_flow_1.json",
    #"path_to_channel": "/home/berg/GitHub/mas-infrastructure/src/cpp/common/_cmake_debug/channel",
    "path_to_channel": "/home/berg/GitHub/monica/_cmake_debug/mas-infrastructure/common/channel",
    "path_to_out_dir": "/home/berg/GitHub/Safe-H2O-Farm/out/",
}
...
try:
    ...
    while True:
        ...
        if ((out_process_id in process_id_to_Popen_args or out_process_id in iip_process_ids)
            ...
            # the out port belongs to an IIP process
            if out_process_id in iip_process_ids:
                ...
                # check for an array port (name should end in _1, _2, ...)
                # numbers are currently ignored and are just for the user to distinguish between ports
                out_port_name_parts = out_port_name.split("_")
                if len(out_port_name_parts) > 1 and out_port_name_parts[-1].isdigit():
                    out_port_name_2 = "_".join(out_port_name_parts[:-1])
                    if out_port_name_2 not in process_id_to_process_srs[out_process_id]:
                        process_id_to_process_srs[out_process_id][out_port_name_2] = \
                            f"--{out_port_name_2}_out_srs=".replace('out_out_', 'out_')
                    else:
                        process_id_to_process_srs[out_process_id][out_port_name_2] += "|"
                    process_id_to_process_srs[out_process_id][out_port_name_2] += info.writerSRs[0]
                    del process_id_to_process_srs[out_process_id][out_port_name]
                else:
            ...
            #print(f"{__file__}: process_id_to_process_srs: {process_id_to_process_srs}")
    ...
    print(f"{__file__}: all components finished")

    for channel in channels:
        channel.terminate()
    print(f"{__file__}: all channels terminated")
    ...
    print(f"Exception {__file__} terminated early:", e)
